...
export function JinjasMotion(data: any, { sText, selected, color3 }: JinjasProps) {
	...
	return (
		<AbsoluteFill
			style={{ backgroundColor: color3, width: width, height: height }}
		...
		>
			...
			<Text_A_Selected
				props={{ text: data.shortS?.text, x: animation, userX: 0, ...data.shortS }}
			/>

			{selected ? (
				<Text_A_Selected props={{ text: data.shortS?.text, x: animation, userX: 0 }} />
			) : (
