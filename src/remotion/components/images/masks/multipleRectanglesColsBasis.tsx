...
export const MultiRectColsMask: React.FC<{
	...
	const rectangleHeight = ({ height, type }: { height: number; type: rectAni }): number => {
		...
		if (type === 'height')
			return interpolate(progress, [0, 1], [START_POS, height], {
				extrapolateLeft: 'clamp',
				extrapolateRight: 'clamp',
				easing: Easing.ease,
			});
		else if (type === 'keepUp')
			return interpolate(progress, [0, 1], [height1 / 2, 0], {
				extrapolateLeft: 'clamp',
				extrapolateRight: 'clamp',
				easing: Easing.ease,
			});
		else
			return interpolate(progress, [0, 1], [height1 / 2, (height1 - height2) / 2], {
				extrapolateLeft: 'clamp',
				extrapolateRight: 'clamp',
				easing: Easing.ease,
			});
	...
	return (
		...
		>
			...
			>
				<defs>
					<clipPath id="myClip">
						{new Array(clipRectangles).fill(true).map((_, i) => {
							return i % 2 === 0 ? (
								<rect
									key={i}
									width={BAR_SPACE}
									y={rectangleHeight({ height: height1, type: 'keepUp' })}
									height={rectangleHeight({ height: height1, type: 'height' })}
								...
								<rect
									key={i}
									width={BAR_SPACE}
									y={rectangleHeight({ height: height1, type: 'keepUpSmaller' })}
									height={rectangleHeight({ height: height2, type: 'height' })}
									x={i * RECT_SPACE + vectorMargin}
									rx={40}
								/>
							);
