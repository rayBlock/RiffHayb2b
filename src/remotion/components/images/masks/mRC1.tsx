...
export const MultiRectColsMask1: React.FC<{
    ...
    const rectangleHeight = ({ h, type }: { h: number, type: rectAni }): number => {
        ...
        if (type === "height")
            return interpolate(progress, [0, 1], [START_POS, h], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.ease })

        else return interpolate(progress, [0, 1], [height / 2, (height - h) / 2], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.ease })
    }
