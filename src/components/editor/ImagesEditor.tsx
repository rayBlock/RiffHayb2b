...
export const ImagesEditor = ({ positionData, propsState, positionAction, propsActions, currentRiff, mainWindow }: ColorBarProps) => {
  ...
  const arrangedPropsFields = [
    `md:w-[30vw] lg:top-14 lg:right-4 md:bottom-24 md:h-1/4 md:w-full md:grid-cols-8 lg:grid-cols-3 md `,
    `lg:top-14 lg:right-4 md:h-10 md:grid-flow-row `,
    `lg:top-14 lg:right-4 md:h-10`,

  ]

  //  const position =  positionData.menu.texts ? arrangedPropsFields[1] :arrangedPropsFields[0]
  const position = positionData.menu.colors && positionData.menu.texts ? arrangedPropsFields[2] :
  ...
  return (
    ...
    <div
      ...
      className={clsx(imagesShown, position, "p-4 absolute  md:gap-2  rounded-3xl lg:grid-cols-3  rounded-tr-none border-2 border-black shadow-[1px_4px_0px_#000000] cursor-pointer z-10  bg-[#d3e0f6]")}>

      {updatedImages.map((imageItem, index) => (


        <button key={index} className={clsx('z-20 hover:border-2  border-black ')} onClick={() => handleModal(imageItem)}>
