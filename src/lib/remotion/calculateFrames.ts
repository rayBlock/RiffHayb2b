import type { FramesCalculationInput,FrameSchema } from '../../types/calculateFrames';
...
function calculateTotalDurations(
	...
	const totalDurationsAllProps = props.reduce((acc, prop) => acc + prop.durationInFrames, 0);
...
function setUnlockedProps(unlockedProps: FrameSchema[], extraFrames: number): FrameSchema[] {
	const extraFramesPerUnlockedProp = Math.round(extraFrames / unlockedProps.length);
...
function subtractFrames(props: FrameSchema[], addFrames: number): FrameSchema[] {
...
function addFramesToRiff(props: FrameSchema[], addFrames: number): FrameSchema[] {
	// Input validation
...
export function calcFramesOld({
	...
	switch (true) {
		case durationDifference > 0:
			...
			const remain = calculateRemainingFrames(
				riffProps,
				seekedDuration,
				totalDurationsLocked,
				durationTraverse
		...
		case durationDifference < 0:
			...
			const remainingNegatives = calculateRemainingFrames(
				riffPropsNegative,
				seekedDuration,
				totalDurationsLocked,
				durationTraverse
